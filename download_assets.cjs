...
const chains = [
    ...
    { name: 'manta.png', url: 'https://cryptologos.cc/logos/manta-network-manta-logo.png?v=029' },
...
chains.forEach(chain => {
    ...
    https.get(chain.url, function (response) {
        if (response.statusCode === 200) {
            response.pipe(file); // Pipe the response stream to the file
            file.on('finish', () => {
            ...
            file.close(); // Close the file stream
            fs.unlink(path.join(destDir, chain.name), () => { }); // Delete the partial file
        }
